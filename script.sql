...
CREATE OR REPLACE FUNCTION calculate_food_cost()
    ...
    $body$
        ...
        BEGIN
            select foodprice into onefood_cost from foodtype where foodtypeid=NEW.foodtypeid;
...
CREATE OR REPLACE FUNCTION get_vacant_roomtypes(
    ...
    $body$
        ...
        BEGIN
            ...
            WHERE roomtype.GuestNum = CalculatedGuestNum OR NoLimit
            GROUP BY roomtype.RoomTypeID, roomtype_branch.rentalprice HAVING COUNT(*) >= InputRoom OR NoLimit;
...
CREATE OR REPLACE FUNCTION get_statistics(
    ...
    $body$
        ...
        BEGIN
            ...
            SUM(bktemp.rentalcost) + SUM(bktemp.foodcost)
...
CREATE OR REPLACE FUNCTION get_rooms_with_bookings(
        inputbranch varchar,
        inputyear integer,
        inputmonth integer
    ) RETURNS TABLE (
    ...
    $body$
        ...
        BEGIN
            ...
            SELECT roomtype.*, count(*) as count_room, count(*)*current_month_day as total_slot, jsonb_agg(jsonb_build_object(
            ...
            -- thêm cái primary key này vô để sau đó chỉ cần group by roomtype_temp.roomtypeid,
            -- nếu ko thì sẽ phải group by tất cả attribute của roomtype_temp
            ALTER TABLE roomtype_temp ADD PRIMARY KEY (roomtypeid);

            RETURN QUERY
            SELECT roomtype_temp.*, SUM(br_join.num_days) as count_slot, 
            ...
            -- ON roomtype_temp.roomtypeid = br_join.roomtypeid
...
CREATE OR REPLACE PROCEDURE checkout(inputbookingid varchar, json_data json)AS
    $body$ 
    DECLARE
        booking_record json;
        food_record json;
    BEGIN
        ...
        LOOP
            -- Iterate through each food item consumed in the booking
            FOR food_record IN SELECT json_array_elements(booking_record->'inputfoodconsumed')
            LOOP
                -- Insert data into the foodconsumed table
